...
import { blogs } from "./blog-data"; // Import your blog data
...
export default function BlogList() {
  ...
  const handleLike = (blogTitle: string) => {
    if (!likes[blogTitle]) {
      setLikes((prevLikes) => ({
        ...prevLikes,
        [blogTitle]: true, // Mark the blog as liked
      }));
    }
  };
    ...
    <section className="p-4 text-neutral-900 dark:text-neutral-100">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`px-2 py-1 rounded-md transition-colors duration-300 
              ${sortOrder === "latest" ? "bg-neutral-800 text-neutral-100" : "bg-neutral-200 text-neutral-900 dark:bg-neutral-700 dark:text-neutral-200"}`}
          >
            Latest
          </button>
          <button
            ...
            className={`px-2 py-1 rounded-md transition-colors duration-300 
              ${sortOrder === "oldest" ? "bg-neutral-800 text-neutral-100" : "bg-neutral-200 text-neutral-900 dark:bg-neutral-700 dark:text-neutral-200"}`}
          >
            Oldest
          </button>
          {/* Search Bar */}
      ...
      <div className="space-y-4">
        {sortedBlogs.slice(0, visibleBlogs).map((blog, index) => (
          <a
            key={index}
            href={blog.url} // Link to the specific blog URL
          ...
          >
            ...
            <div className="flex items-center justify-end mt-2">
              <button
                ...
                className={`text-lg ${likes[blog.title] ? "text-red-500" : "text-gray-400"}`}
              >
                {likes[blog.title] ? "❤️" : "🤍"}
              </button>
            </div>
        ...
        {visibleBlogs < sortedBlogs.length && (
          <div className="flex justify-center mt-4">
            <button onClick={loadMoreBlogs} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700">
              Read More
            </button>
          </div>
        )}
